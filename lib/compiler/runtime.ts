...
async function loadDekaCompiler(): Promise<WasmCompiler> {
  const manifestResponse = await fetch(DEKA_COMPILER_ARTIFACT_PATH, { cache: 'no-cache' });
  ...
  const wasmUrl = new URL(manifest.artifact.file, new URL(DEKA_COMPILER_ARTIFACT_PATH, window.location.origin));
